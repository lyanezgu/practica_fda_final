...
train <- train %>% group_by(prov_state) %>% 
  mutate(mean_copagos_state=mean(copagos))
...
train <- train %>% group_by(codigo_enf) %>% 
  mutate(mean_copagos_enf=mean(copagos))
...
train<-train %>%
    mutate(categoria_state = case_when(mean_copagos_state >= 1250 ~ 'High',
                                  mean_copagos_state >= 1150 ~ 'Mid',
                                  TRUE ~ 'Low'))
train
...
train<-train %>%
    mutate(categoria_enf = case_when(mean_copagos_enf >= 1170 ~ 'High',
                                  mean_copagos_enf >= 1000 ~ 'Mid',
                                  TRUE ~ 'Low'))
train
...
### 6.4.2. Top 10 enfermedades más comunes detectadas
...
### 6.4.3. Top 10 enfermedades más caras 
...
### 6.4.4. Gráfico de calor - ratio de cobertura por Estado para las 10 enfermedades más caras
...
### 6.4.5. Correlación entre variables
...
### 6.4.6. Análisis de la distribución de las variables
...
### 6.4.7. Boxplot - análisis de la variables de relevancia y de los atípicos observados
...
### 6.4.8. Análisis de correlación entre variables
```{r}
