...
#ggplot(train, aes(drg_def)) + geom_bar()

#ggplot(train, aes(x=drg_def)) + geom_histogram(stat='count')


```

```{r diez mas comunes, message=FALSE, warning=FALSE, include=TRUE, list(width.cutoff=60), tidy=TRUE, tidy.opts=list(width.cutoff=60)}

new <- train %>%group_by(drg_def) %>%summarize(n = n())%>%arrange(-n)%>%head(10)
new
```
...
ggplot(train, aes(drg_def,prov_state, fill=cobertura))+geom_tile() #sad
