...
El conjunto de datos "Hospital Charges in America" contiene 12 columnas y 163.065 fila y lo obtenemos en formato .csv. 
...
Para la imputación de datos faltantes en las columnas mean_covered_charges y total_discharges, reemplazados todos sus NAs según los valores medios de las mismas. Con la función summary si comproba que ya no hay más datos faltantes en el data set train.
...
Realizamos una división de la columna "drg_ref". Separamos la columna en dos diferenciando entre código de la enfermedad y descripción de la enfermedad. Nos servirá a futuro para simplificar el análisis y visualización de los datos de interés en graficos y tablas.
...
El grupo ha decidido crear una nueva variable representativa del valor de los copagos que debe realizar el paciente o su seguro privado (en caso de contar con uno), para completar (junto a lo que cubre el Estado con el Medicare) el coste total de la intervención hospitalaria.
...
Inicialmente hemos creado una nueva columna mean_total_payments agrupada por state (es decir, la media de total_payments por estado) para que así, estados iguales tengan el mismo valor quantitativo en esta variable. 
...
Lo mismo hicimos para la variable enfermedad donde creamos una nueva variable que es la media total de los costes por enfermedad.
...
### 6.2.1. Gráfico EEUU: valor de los Copagos por Estados

Con la ayuda de la libreria "usmap" hemos creado un mapa para visualmente identificar como se distribuyen los valores de copagos en los diferentes estados de EEUU.
...
### 6.2.2. Gráfico EEUU: valor de los Costes Médicos por Estados

Con la misma libreria "usmap" cream0os un nuevo mapa para visualmente identificar como se distribuyen los valores de total_payments en los diferentes estados de EEUU.
...
plot_usmap(data = statepop3, values = "mean_total_cost", color = "white") + 
  ...
  theme(legend.position = "right")
```
Los estados americanos con los costes médios totales más elevados son Alaska (media coste total de 14.928 dólares), DC - Washington (media coste total de 13.382 dólaress) y Hawái (media coste total de 12.996 dólares).

Comparando los dos gráficas se puede concluir que estados como Hawái y Alaska tienen tanto un coste médio total de tratamiento alto, cuanto de copagos a serem pagos por el paciente. Wyoming, el estado más caro para el paciente, es también el quinto estado con tratamiento hospitalar total medio más caro.

### 6.2.3. Top 10 enfermedades más comunes detectadas
...
### 6.2.4. Top 10 enfermedades más caras 
...
### 6.2.5. Gráfico de calor - ratio de cobertura por Estado para las 10 enfermedades más caras
...
### 6.2.6. Correlación entre variables

Queremos analisar si se puede afirmar estadisticamente que hay correlación positiva entre las variables mean_total_payments y mean_medicare_payments.
...
```
Através de la grafica y de la función cor (valor 0.99 aproximadamente), afirmamos que estás dos variables se correlacionan positivamente. Eso era esperado una vez que el valor mean_medicare_payments es parte de lo que se entende por mean_total_payments.

### 6.2.7. Análisis de la distribución de las variables
...
Analisamos por fin si la variable "copagos" sigue una distribución normal.
...
```
Graficamente podemos decir que las tres curvas se asemejan a una distribuición normal ligeramente concentrada a la izquierda.



### 6.2.8. Boxplot - análisis de la variables de relevancia y de los atípicos observados

Creamos un sub data set solo con las variables numéricas para analisar si tienes valores atípicos.
...
### 6.2.9. Análisis de correlación entre variables númericas
...
```
Se observa una correlación positiva en casi todos los casos, expecto para copagas y cobertura.
...
Vemos la información del elegido como "mejor"
```{r,include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
summary(seleccion)
```
...
### Nota:
Existe una forma de ver un resumen de si se cumplen o no los supuestos, pero es mejor hacerlo paso a paso.
...
Se puede graficar todo a la vez.
```{r,include=TRUE, message=FALSE, warning = FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
plot(mejor_modelo)
```
