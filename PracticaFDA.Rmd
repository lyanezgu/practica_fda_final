...
El conjunto de datos elegido por el grupo se llama "Hospital Charges in America" y incluye información que compara las tarifas y costes de los servicios de hospitalizacón en diferentes Estados de los EEUU para los 100 principales grupos de diagnóstico de enfermedades. 
...
Los datos faltantes han sido imputados exclusivamente en las columnas que no creemos que no van a servir de análisis principal para este estudio (total_discharges y mean_covered_charges), para así intentar que la predicción que hagamos sea lo más precisa posible.
...
Para la imputación de datos faltantes en las columnas mean_covered_charges y total_discharges, se ha decidido reemplazar todos sus NAs según los valores medios de las mismas variables.
...
Una vez entendidas las variables de nuestro conjunto de datos y teniendo claros nuestros objetivos de estudio, pasamos a la parte de análisis en profundidad de nuestros datos, para tratar de entenderlos y ver como se relacionan unas variables con otras.
...
Primero de todo realizamos una división de la columna "drg_ref". Separamos la columna en dos, diferenciando entre código de la enfermedad y descripción de la enfermedad. Nos servirá a futuro para simplificar el análisis y visualización de los datos de interés en gráficos y tablas.
...
Una vez que hemos creado nuestras nuevas colunmas cuantitativas y para reducir los niveles del predictor categóricos a 3, creamos una segunda nueva variable agrupando los costes por Estados (51 estados) por 3 niveles High, Mid y Low según la variable "mean_total_state" recientemente creada. El hecho de reducir y agrupar nuestras variables categóricas de interés en 3 niveles, nos facilitará el análisis de nuestros datos.
...
* **Low o coste bajo:** para los Estados con un coste total en media inferior a 8.500 dólares (colores verdes más intensos)
...
* **High o de coste alto:** para las Enfermedades con un coste total en media igual o mayor a 15.000 dólares.

* **Mid o de coste medio:** para las Enfermedades con un coste total en media igual o mayor a 5.000 dólares y menor a 15.000 dólares.

* **Low o coste bajo:** para las Enfermedades con un coste total en media inferior a 5.000 dólares.
...
### 6.2.1. Gráficos de mapa de EEUU: valor de los Copagos, Costes Médicos por Estado y su comparativa
...
El grupo DRG de mayor frecuencia es "194 - Neumonía y Pleuresía con complicaciones/patolog
ias previas" (nivel medio de severidad) con 2.424 casos. En segundo y tercer lugar tenemos las "690 - Infecciones de las vías urinarias sin complicaciones/patologías previas" (2.400 casos) y "392 -Esofagitis, Gastroenteritis y Enfermedades digestivas sin complicaciones/patologías previas" con (2.368 casos).

### 6.2.3. Top 10 enfermedades más caras 

Creamos un grafico de barras para visualizar las 10 enfermedades más caras en media de nuestro data set (cada enfermedad agrupada por la media de mean_total_payments).
...
El grupo DRG más caro es "870 - Septicemia y sepsis grave con periodo de hospitalización superior a 96 horas". Una septicemia occure cuando el sistema inmunitario se descontrola y ataca a sus proprios órganos y tejidos. Es la complicación de una infección, siendo una urgencia médica que requiere tratamiento inmediato. El coste media del tratamiento en los EEUU es de 44.467 dólares. 

El segundo y tercer grupo DRG más caros son las "853 - Infecciones y enfermedades parasitarias" (coste médio de 40.296 dólares) y "207- Enfermedades respiratorias que requieren ventilación respiratória por más de 96 horas consecutivas" (coste médio de 38.372 dólares).	

### 6.2.4. Gráfico de calor - ratio de cobertura por Estado para las 10 enfermedades más caras

Creamos un grafico de calor para visualizar el ratio de cobertura por Estado para las 10 enfermedades más caras de nuestro data set.
...
De manera general se puede decir que la tasa de cobertura de las top 10 enfermedades más caras es, en todos los estados de Estados Unidos, superior a 80% (colores azules oscuro predominantes en casi todo el gráfico de calor).
...
En Wyoming, el Estado donde se pagan las mayores cifras de copagos (mayor coste personal para el enfermo o para su seguro privado en caso de tenerlo), no han sido registrados casos de Septicemia o Enfermedades Respiratórias, mientras la tasa de cobertura para Enfermedades Parasitárias es de 82%.


### 6.2.5. Correlación entre variables

Una vez vistas y analizadas de forma individual las principales variables que componen nuestros datos, es interesante pasar a realizar un análisis conjunto entre ellas para ver como se relacionan entre si.

#### 6.2.5.1. Correlación: mean_total_payments y mean_medicare_payments

Queremos analizar si se puede afirmar estadísticamente que hay correlación positiva o negativa entre las variables mean_total_payments y mean_medicare_payments.
...
A través de la gráfica y de la función "cor" (valor 0.99 obtenido aproximadamente), afirmamos que estás dos variables se correlacionan positivamente en un grado muy alto. Se aprecia que a nivel general en la gran mayoría de casos, cuando aumenta el coste (sube la variable "mean total payments"), aumenta la cobertura del Medicare (sube también la variable "mean medicare payments").

#### 6.2.5.2. Correlación: mean_total_payments y copagos

Ahora pasamos a analizar si se puede afirmar estadisticamente que hay correlación positiva o negativa entre las variables mean_total_payments y copagos.
...
A través de la gráfica y de la función "cor" (valor 0.38 obtenido aproximadamente), afirmamos que estás dos variables se correlacionan positivamente a un nivel medio-bajo. No se puede afirmar que en la gran mayoría de casos, cuando aumenta el coste de la enfermedad, aumenta en la misma medida el copago que debe pagar el paciente.

#### 6.2.5.3. Correlación: mean_medicare_payments y copagos

Analizamos también si se puede afirmar estadisticamente que hay correlación positiva o negativa entre las variables mean_medicare_payments y copagos.
...
A través de la gráfica y de la función "cor" (valor 0.24 obtenido aproximadamente), afirmamos que estás dos variables se correlacionan positivamente a un nivel medio-bajo. No se puede afirmar que en la gran mayoría de casos, cuando aumenta la cobertura de la enfermedad por parte de la seguridad social, aumenta en la misma medida el copago que debe pagar el paciente. La tasa de cobertura no es igual siempre, sino que varía entre los diferentes tipos de enfermedad y por ello los copagos no son siempre iguales en proporción.

#### 6.2.5.4. Correlación: cobertura y copagos


Por último, estudiamos si se puede afirmar estadisticamente que hay correlación positiva o negativa entre las variables cobertura y copagos.
...
A través de la gráfica y de la función "cor" (valor -0.43 obtenido aproximadamente), afirmamos que estás dos variables se correlacionan negativamente a un nivel medio-bajo. No se puede afirmar que en la gran mayoría de casos, cuando aumenta la tasa de cobertura de la enfermedad por parte de la seguridad social, disminuya en la misma medida el copago que debe pagar el paciente. La tasa de cobertura no es igual siempre, sino que varía entre los diferentes tipos de enfermedad y por ello los copagos no son siempre iguales en proporción. 
...
También esinteresante de cara al estudio y a la consecución de sus objetivos, ver que distribución siguen los datos de cada variable de interés dentro del data set.

#### 6.2.6.1. Distribución variable mean_total_payments

Primero verificamos si la variable **"mean_total_payments"** sigue una distribución normal.
...
Ahora queremos ver si la variable **"mean_medicare_payments"** sigue de la misma forma una distribución normal.
...
Pasamos a analizar de la misma forma si la variable **"copagos"** sigue una distribución normal.
...
Por último estudiamos si la variable **"cobertura"** sigue también una distribución normal.
...
Graficamente podemos decir que las tres primeras curvas de distribución mostradas para las variables mean_total_payments, mean_medicare_payments y copagos, se asemejan a una distribuición normal ligeramente concentrada a la izquierda (con valores de mayor coste en la cola derecha de la distribución, los cuales serán más raros, infrecuentes y extremos). 

En el caso de la última distribución mostrada referente a la variable cobertura, se ve que pasa justo lo contrario, la mayor parte de valores se ubican a la derecha de la distribución (en las tasas de cobertura médica de entre un 50% y un 100%), siendo las tasas de cobertura inferiores al 50% ubicadas en la cola izquierda de la distribución, valores más infrecuentes.


### 6.2.7. Boxplot - análisis de la variables de relevancia y de los atípicos observados

Creamos un sub data set solo con las variables numéricas para analizar si tienen valores atípicos y analizar sus valores medios y sus intervalos de confianza, a tarvés de gráficos de tipo Boxplot.
...
Se observa que la variable mean_total_payments tiene su valor medio en 9.715 dólares aproximadamente independientemente del hospital, su localización y el tiempo de enfermedad en cuestión. Además la variable se mueva en la mayoría de casos en valores cercanos a esa media (entre 5.000 y 15.000 dólares), aunque se pueden observar atípicos que van desde 20.000 dólares hasta por encima de los 150.000 dólares.
...
La cobertura de Medicare o Seguridad Social americana tiene su media alrededor de 8.500 dólares, con la mayor parte de valores concentrados entre los 4.000 y 12.000 dólares. Se pueden observar datos atípicos en el conjunto, ya que en determinados casos de enfermedad en algunos Estados se dan coberturas por encima de los 50.000, 100.000 y 150.000 dólares de forma excepcional.
...
La variable copagos (lo que paga el paciente y/o su seguro privado por la enfermedad) tiene una media de 1.200 dólares aproximadamente, con valores en los datos muy concentrado alrededor de esa media en general. pero se observan al mismo tiempo atípicos por debajo de los 1.000 dólares, y atípicos por encima de 20.000, 40.000 y 60.000 dólares de copago (dependiendo del Estado, el hospital, el tipo de enfermedad y la gravedad del paciente).
...
La tasa de cobertura analizada se situa en media entre valores de un 84-85%, y en su mayoria de casos en el rango de un 80-95% de cobertura. Se observan de la misma manera valores atípicos por debajo del 70% pudiendo a llegar la cobertura por debajo del 10% del coste total hospitalario de la enfermedad.

### 6.2.8. Análisis de correlación entre variables númericas

Para finalizar con el análisis de las distintas variables del data set y el estudio de como se relacionan entre si, queremos ver de forma global como se correlacionan las variables numéricas que nos pueden llegar a servir para el modelo de predicción.
...
```
Se observa una correlación positiva en casi todos los casos (en algunos casos alta como para mean_total_payments y mean_medicare_payments, y en otros casos baja, como para mean_total_state y cobertura), excepto como ya habiamos visto para copagos y cobertura, que tienen una correlación media negativa.
...
Una vez analizado en profundidad nuestro conjunto de datos y habiendo entendido y tranformado nuetras variables, trataremos de ajustar un modelo de regresión lineal múltiple que trate de predicir el valor de los copagos para el paciente en base a las variables que mejor anticipen ese valor.

## 7.1. Ajuste del modelo de regresión

Ajustamos un modelo de regresión lineal mútiple con el que vamos a predecir el valor de la variable copagos a partir de las siguientes variables independientes(categoria_state, categoria_enf,cobertura, mean_covered_charges y total_discharges) seleccionadas en base a los análisis y estudios de correlación vistos con anterioridad.
...
* El conjunto de datos seleccionado tiene excesivas variables categóricas redundantes sobre la localización de los hospitales y pocas variables númericas de interes para el análisis y la predicción.

* Las transformaciones de datos posibles, dadas las limitaciones surgidas por la escasez de variables numéricas de interés, son también limitadas y complejas. Se requeriría de técnicas o métodos de transformación de variables más avanzados para intentar ver si se puede sacar nueva información de relevancia con esas transformaciones.

* Vemos una falta de adecuación y ajuste del modelo de regresión lineal múltiple obtenido. Se observa un modelo con unos residuos que no siguen una distribución normal, con heterocedasticidad (varianza no constante en el modelo - se viola la homocedasticidad y hay un claro patrón de embudo) y que no predice de forma adecuada la variable respuesta o dependiente, en base a las variables explicativas o independientes. 

* Al haberse considerado finalmente el modelo como no óptimo en la fase de "training" ya que no se ajusta bien a los datos, no se ha probado su validez en la fase de "test", ya que es un modelo considerado como no válido para el conjunto de datos.

* Viendo los resultados obtenidos, el grupo considera un inconveniente continuar con este conjunto de datos a futuro ya que somos conscientes de que requiere de un profundo e intensivo trabajo de procesado de datos, que a la postre no asegura la obtención de resultados óptimos y de interés para el estudio de la adecuación y ajuste de otros modelos de Machine Learning.
