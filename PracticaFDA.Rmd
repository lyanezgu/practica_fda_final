...
Creamos nueva variable agrupando los Estados por niveles según la variable "mean_total_state" (= Media por Estado según mean_total_payments)
```{r}
...
Creamos nueva variable agrupando las enfermedades por niveles según la variable "mean_total_enf" (= Media por enfermedad según mean_total_payments)
```{r}
train<-train %>%
...
* Queremos ver si la variable **"mean_total_payments"** sigue una distribución normal
```{r, message=FALSE, warning=FALSE, include=TRUE}
train%>%
...
Analizamos la normalidad de la misma variable pero utilizando logaritmos
```{r, message=FALSE, warning=FALSE, include=TRUE}
train%>%
  filter(mean_total_payments<100000 ) %>%
...
* Ahora queremos ver si la variable **"copagos"** sigue una distribución normal
```{r,message=FALSE, warning=FALSE, include=TRUE}
train%>%
  filter(mean_medicare_payments<100000 ) %>%
