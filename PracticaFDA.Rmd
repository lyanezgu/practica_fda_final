...
hospital_charges
names(hospital_charges) <- c('drg_def', 'prov_name', 'prov_city', 'prov_zip', 'referral_reg', 'total_discharges', 'mean_total_payments', 'mean_medicare_payments','prov_id', 'prov_address','prov_state', 'mean_covered_charges')
...
## 5.3.2 Eliminar dolar de las ultimas 3 columnas 
...
hospital_charges <- hospital_charges %>% mutate(copagos = mean_total_payments - mean_medicare_payments)
hospital_charges
```
...
region_geog <- hospital_charges %>% group_by(prov_state) %>% summarise (mean_total_price = mean(mean_total_payments))
...
plot_usmap(data = statepop, values = "mean_total_price", color = "white") + 
  scale_fill_continuous(low='light blue', high ='dark blue',name = "Mean Total Payments", label = scales::comma) + 
