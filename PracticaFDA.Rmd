...
#-----------------------------------parte regresion luisa---------------------#

# 8. Regresion lineal multiple

## 8.1. Ajuste de modelo de regresión

Ajustamos un modelo de regresión lineal mútiple con el que vamos a predecir el valor de la variable copagos a partir de las siguientes variablesindependientes(categoria_state,categoria_enf,cobertura+mean_covered_charges,y total_discharges).
```{r}
...
Para la selección de variables se utiliza el método de la selecció por pasos
```{r}
library(MASS)
...
```{r}
#indico cual es el mejor mocelo q se selelccionó con el método usado anteriormente
...
Primero se comprueba la normalidad de los residuos, pero al usar Shapiro test solo permite usar las 5000 perimeas muestras de los residuos, así que también uso Anderson-Darling para comparar resultados. Como se comprueba mas adelante el p_value < 0.05 por tanto se rechaza la hipotesis nula de que los residuos siguen una distriibución normal.
...
### Homocedasticidad
Vamos a comprobar la hocedasticidad( que los residuos tengan una varianza constante).
...
Se puede comprobar la independencia de los residuos gráficamente y como se observa no se ven patrónes extraños y esto nos puede indicar que hay independencia en los residuos.
